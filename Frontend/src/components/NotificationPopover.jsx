...
import { X, Check, Info, BellRing,ShieldCheck,ChevronRight,UserCheck   } from "lucide-react";
    ...
    <div className="flex flex-col items-center text-center">
      ...
      <div className="mt-4 space-y-3">
        <p className="text-slate-400 text-xs leading-relaxed">
          <span className="text-indigo-400 font-bold">{actionModal.sender?.name}</span> ({actionModal.sender?.email}) 
          invited you to join:
        </p>
...
{infoModal && (
  <ModalWrapper onClose={() => setInfoModal(null)}>
    <div className="flex flex-col items-center text-center">
      {/* Dynamic Icon Header */}
      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 border ${
        infoModal.type === 'ACCESS_UPGRADED' ? 'bg-emerald-500/10 border-emerald-500/20' :
        infoModal.type === 'INVITE_RECEIVED' && infoModal.status === 'DECLINED' ? 'bg-rose-500/10 border-rose-500/20' :
        'bg-slate-800 border-slate-700'
      }`}>
        {infoModal.type === 'ACCESS_UPGRADED' ? <ShieldCheck className="text-emerald-500" size={30} /> :
         infoModal.type === 'COLLAB_JOINED' ? <UserCheck className="text-indigo-500" size={30} /> :
         <Info className="text-slate-400" size={30} />
        }
      </div>

      <h3 className={`text-lg font-black uppercase tracking-tighter ${
        infoModal.type === 'ACCESS_UPGRADED' ? 'text-emerald-400' : 'text-white'
      }`}>
      ...
      <div className="mt-4 w-full bg-slate-950/50 border border-slate-800 p-5 rounded-2xl text-left">
        <div className="space-y-4">
          {/* Resource Detail */}
          <div>
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">Resource Context</p>
            <p className="text-sm font-bold text-slate-200">{infoModal.resourceName}</p>
          </div>

          {/* Upgrade Logic Visual */}
          {infoModal.type === 'ACCESS_UPGRADED' && (
            <div className="flex items-center gap-3 py-2 border-t border-slate-800">
              <div className="text-[10px] font-black text-slate-500 line-through">{infoModal.oldMode}</div>
          ...
          {infoModal.status !== 'READ' && infoModal.status !== 'PENDING' && (
             <div>
               ...
               <span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase border ${
                 infoModal.status === 'DECLINED' ? 'bg-rose-500/10 text-rose-500 border-rose-500/20' : 'bg-indigo-500/10 text-indigo-400 border-indigo-500/20'
               }`}>
                 {infoModal.status}
               </span>
             </div>
          ...
          <div className="pt-3 border-t border-slate-800">
            <p className="text-[10px] text-slate-400 leading-relaxed italic">
              "{infoModal.meta?.message || infoModal.message || "No additional details provided."}"
            </p>
          </div>
        </div>
      </div>
